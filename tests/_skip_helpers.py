...
def external_model_skip_reason(exc: BaseException) -> str | None:
    """Return a pytest skip reason for transient/offline model dependency errors."""
    try:
        import httpx
    except ImportError:  # pragma: no cover - httpx is available in test envs that use this
        httpx = None

    try:
        from huggingface_hub.errors import LocalEntryNotFoundError
    except ImportError:  # pragma: no cover - hub package may be absent in some lanes
